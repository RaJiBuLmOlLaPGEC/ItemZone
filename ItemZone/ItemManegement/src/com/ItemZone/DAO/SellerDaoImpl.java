...
public class SellerDaoImpl implements SellerDao{
	...
	public Product addItem(Product p) throws ProductException {
		try (Connection conn= DBUtil.provideConnection()){
			PreparedStatement ps= conn.prepareStatement("insert (pname,price,unit,sellerid,cid) into product values(?,?,?,?,?)");
	...
	public String deleteItem(int sid, int pid) {
		...
		try(Connection conn=DBUtil.provideConnection()) {
			PreparedStatement ps= conn.prepareStatement("delect from product where pid=? AND sellerid=?");
			ps.setInt(1, pid);
			ps.setInt(2, sid);
			int x=ps.executeUpdate();
			if(x>0) {
